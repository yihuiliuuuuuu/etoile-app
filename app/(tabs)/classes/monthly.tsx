...
import { buildYearMonthlyChart } from '@/utils/class-month-activity';
...
function chartMaxForCounts(counts: number[]): number {
  const peak = Math.max(...counts, 1);
  return Math.max(8, Math.ceil(peak / 2) * 2);
}

function yTicksForMax(max: number): number[] {
  const step = max / 4;
  return [max, max - step, max - step * 2, max - step * 3, 0].map((v) => Math.round(v));
}

function classInsightMessage(current: number, previous: number | null): string {
  if (previous === null) {
    return 'Class attendance is off to a start. Keep the momentum going 🔥';
  }
  if (current > previous) {
    return 'Class attendance increased this month. Keep the momentum going 🔥';
  }
  if (current < previous) {
    return 'Class attendance decreased this month. Keep going ✨';
  }
  return 'Class attendance held steady this month. Keep going ✨';
}

export default function MonthlyClassDetailScreen() {
  ...
  const monthlyChart = useMemo(() => buildYearMonthlyChart(entries), [entries]);
  const currentMonthChartIndex = monthlyChart.length - 1;

  const activeIndex = selectedBarIndex ?? currentMonthChartIndex;
  const activeMonth = monthlyChart[activeIndex]!;

  const chartMax = useMemo(
    () => chartMaxForCounts(monthlyChart.map((m) => m.count)),
    [monthlyChart],
  );

  const insightSubtitle = useMemo(() => {
    const current = monthlyChart[activeIndex]!.count;
    const prevMonth = activeIndex > 0 ? monthlyChart[activeIndex - 1]!.count : null;
    return classInsightMessage(current, prevMonth);
  }, [activeIndex, monthlyChart]);

  const insightBars = useMemo(() => {
    const idx = activeIndex;
    const thisMonth = monthlyChart[idx]!;
    const lastMonth = idx > 0 ? monthlyChart[idx - 1]! : null;
    const thisCount = thisMonth.count;
    const lastCount = lastMonth?.count ?? 0;
    const denom = Math.max(thisCount, lastCount, 1);
    return {
      thisCount,
      lastCount,
      thisLabel: thisMonth.label,
      lastLabel: lastMonth?.label ?? '—',
      thisWidthPct: (thisCount / denom) * 100,
      lastWidthPct: (lastCount / denom) * 100,
    };
  }, [activeIndex, monthlyChart]);
  ...
  const barHeights = useMemo(
    () =>
      monthlyChart.map((m) =>
        Math.max(8, (m.count / chartMax) * CHART_INNER_HEIGHT),
      ),
    [monthlyChart, chartMax],
  );

  const chartPlotInnerWidth =
    SIDE_PAD * 2 + monthlyChart.length * BAR_WIDTH + (monthlyChart.length - 1) * BAR_COLUMN_GAP;
  ...
  const scrollChartToCurrentMonth = useCallback(() => {
    ...
    const barCenter =
      edge +
      SIDE_PAD +
      currentMonthChartIndex * (BAR_WIDTH + BAR_COLUMN_GAP) +
      BAR_WIDTH / 2;
    ...
    didInitialChartScroll.current = true;
  }, [chartScrollWidth, currentMonthChartIndex, monthlyChart.length]);

  const periodSubtitle =
    activeIndex === currentMonthChartIndex ? 'This month' : activeMonth.longLabel;
  ...
  return (
    <View style={[styles.screen, { paddingTop: insets.top + 8 }]}>
      <View style={styles.topBar}>
        ...
        <Text style={styles.screenTitle}>Monthly Class</Text>
      ...
      >
        <View style={styles.currentBlock}>
          <Text style={styles.countBig}>{activeMonth.count}</Text>
                    ...
                    {monthlyChart.map((month, i) => {
                      ...
                      return (
                        <View key={month.monthIndex} style={styles.barColumn}>
                          ...
                          <Pressable
                            accessibilityRole="button"
                            accessibilityLabel={`${month.label}, ${month.count} classes`}
        ...
        <View style={styles.highlightCard}>
          ...
          <View style={styles.highlightRows}>
            <View style={styles.hlRow}>
              <Text style={styles.hlCount}>{insightBars.thisCount}</Text>
              <View style={[styles.hlTrack, styles.hlTrackThisPeriod]}>
                <View style={[styles.hlFillThisPeriod, { width: `${insightBars.thisWidthPct}%` }]}>
            ...
            <View style={styles.hlRow}>
              <Text style={styles.hlCount}>{insightBars.lastCount}</Text>
              <View style={[styles.hlTrack, styles.hlTrackThisPeriod]}>
                <View style={[styles.hlFillLastPeriod, { width: `${insightBars.lastWidthPct}%` }]}>
