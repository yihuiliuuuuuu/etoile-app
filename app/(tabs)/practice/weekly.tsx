...
export default function WeeklyPracticeDetailScreen() {
  ...
  const weeklyChart = useMemo(() => buildYearWeeklyChart(entries), [entries]);
  const currentWeekIndex = Math.max(0, weeklyChart.length - 1);

  const activeIndex = selectedBarIndex ?? currentWeekIndex;
  const activeWeek = weeklyChart[activeIndex] ?? { hours: 0, rangeStr: '', key: 'empty' };

  const chartMax = useMemo(
    () => chartMaxForHours(weeklyChart.map((w) => w.hours)),
    [weeklyChart],
  );

  const insightSubtitle = useMemo(() => {
    const current = weeklyChart[activeIndex]?.hours ?? 0;
    const prev = activeIndex > 0 ? weeklyChart[activeIndex - 1]!.hours : null;
    return practicePeriodInsightMessage(current, prev, 'week');
  }, [activeIndex, weeklyChart]);

  const insightBars = useMemo(() => {
    const thisWeek = weeklyChart[activeIndex];
    const lastWeek = activeIndex > 0 ? weeklyChart[activeIndex - 1] : null;
    const thisH = thisWeek?.hours ?? 0;
    const lastH = lastWeek?.hours ?? 0;
    const denom = Math.max(thisH, lastH, 1e-6);
    const isCurrentWeek = activeIndex === currentWeekIndex;
    return {
      thisHours: thisH,
      lastHours: lastH,
      thisLabel: isCurrentWeek ? 'This week' : (thisWeek?.rangeStr ?? '—'),
      lastLabel: lastWeek ? (isCurrentWeek ? 'Last week' : lastWeek.rangeStr) : '—',
      thisWidthPct: (thisH / denom) * 100,
      lastWidthPct: (lastH / denom) * 100,
    };
  }, [activeIndex, weeklyChart, currentWeekIndex]);
  ...
  const barHeights = useMemo(
    () => weeklyChart.map((w) => Math.max(8, (w.hours / chartMax) * CHART_INNER_HEIGHT)),
    [weeklyChart, chartMax],
  );

  const chartPlotInnerWidth =
    SIDE_PAD * 2 +
    weeklyChart.length * BAR_WIDTH +
    Math.max(0, weeklyChart.length - 1) * BAR_COLUMN_GAP;
  const chartScrollWidth = 20 + chartPlotInnerWidth + 20;
  const scrollMinWidth = Math.max(chartScrollWidth, SCREEN_W - 52);

  const scrollChartToCurrentWeek = useCallback(() => {
    if (didInitialChartScroll.current || weeklyChart.length === 0) return;
    ...
    const edge = 20;
    const barCenter =
      edge + SIDE_PAD + currentWeekIndex * (BAR_WIDTH + BAR_COLUMN_GAP) + BAR_WIDTH / 2;
    ...
    didInitialChartScroll.current = true;
  }, [chartScrollWidth, currentWeekIndex, weeklyChart.length]);

  const periodSubtitle =
    activeIndex === currentWeekIndex ? 'This week' : activeWeek.rangeStr;
      ...
      >
        <View style={styles.currentBlock}>
          <View style={styles.hoursRow}>
            <Text style={styles.hoursBig}>{formatHoursLabel(activeWeek.hours)}</Text>
          ...
          {weeklyChart.length === 0 ? (
            ...
            <View style={styles.chartRow}>
              <ScrollView
                ...
                bounces
                nestedScrollEnabled
                contentContainerStyle={{ minWidth: scrollMinWidth }}
                onContentSizeChange={scrollChartToCurrentWeek}
              >
                <View style={[styles.chartScrollRow, { width: scrollMinWidth }]}>
                  <Pressable
                    style={styles.edgeTap}
                    onPress={clearSelection}
                    accessibilityLabel="Clear bar selection"
                  />
                  <View style={[styles.chartPlot, { width: chartPlotInnerWidth }]}>
                      ...
                      {weeklyChart.map((week, i) => {
                        ...
                        return (
                          <View key={week.key} style={styles.barColumn}>
                            ...
                            <Pressable
                              accessibilityRole="button"
                              accessibilityLabel={`${week.rangeStr}, ${formatHoursLabel(week.hours)} hours`}
        ...
        <View style={styles.highlightCard}>
          <Text style={styles.highlightTitle}>Weekly Insight</Text>
          ...
          <View style={styles.highlightRows}>
            <View style={styles.hlRow}>
              ...
              <View style={[styles.hlTrack, styles.hlTrackThisWeek]}>
                <View style={[styles.hlFillThisWeek, { width: `${insightBars.thisWidthPct}%` }]}>
            ...
            <View style={styles.hlRow}>
              ...
              <View style={[styles.hlTrack, styles.hlTrackThisWeek]}>
                <View style={[styles.hlFillLastWeek, { width: `${insightBars.lastWidthPct}%` }]}>
...
const styles = StyleSheet.create({
  ...
  edgeTap: {
    width: 20,
    height: CHART_INNER_HEIGHT + 8,
    alignSelf: 'flex-end',
  },
  chartPlot: {
    position: 'relative',
    minHeight: CHART_INNER_HEIGHT,
  },
  gridOverlay: {
    position: 'absolute',
    left: 0,
